  ...
  try {
    ...
    const powerBadgePerks = [
      ...
      },
      {
        ...
        requirements: ["Power Badge holder", "Active Warpcast user"],
      ...
      },
      {
        ...
        requirements: ["Power Badge holder", "Base network activity"],
        value: "Variable",
      },
    ];
    ...
    if (supabaseUrl !== "https://placeholder.supabase.co") {
      for (const drop of exclusiveDrops) {
        const { error } = await supabase.from("airdrops").upsert(
          {
...
async function checkActiveCampaigns(): Promise<PowerBadgeAirdrop[]> {
  ...
  try {
    ...
    const nftMints = ["Based Power", "Farcaster OGs", "Early Builders"];
...
async function monitorChannelForPowerBadge(
  channel: string,
): Promise<PowerBadgeAirdrop[]> {
